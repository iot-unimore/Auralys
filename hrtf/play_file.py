...
parser.add_argument("-o", "--output", type=int_or_str, help="output device (numeric ID or substring)")
...
try:
    ...
    stream = sd.OutputStream(
        samplerate=fs, device=args.output, channels=data.shape[1], callback=callback, finished_callback=event.set
    )
    with stream:

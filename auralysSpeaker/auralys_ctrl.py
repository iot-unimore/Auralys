...
_COORD_TYPE_DICT={"ac":"absolute, cartesian","as":"absolute, spherical","rc":"relative, cartesian","rs":"relative, spherical"}
...
def set_position(x,y,z,type):
    ...
    if("ac"==type):
        ...
        [mks_originL, mks_originR, mks_originF]=compute_wires_length(float(mks_origin_x_mm),float(mks_origin_y_mm),float(mks_origin_z_mm))
        ...
        if( ((mks_position_F>maxF) or (mks_position_F<minF)) or ((mks_position_R>maxR) or (mks_position_R<minR)) or ((mks_position_L>maxL) or (mks_position_L<minL)) ):
